...
#' @keywords internal
multiwass <- function(obj1, obj2, newdata1, newdata2, dist.method, dt1, 
                      dt2, ...) {
  npts <- NROW(newdata1)
  wvec <- c()
  ...
  for (k in 1:npts) {
    params1 <- list(mu = pred1$mu[k], sigma = pred1$sigma[k], nu = pred1$nu[k], 
                    tau = pred1$tau[k])
    params2 <- list(mu = pred2$mu[k], sigma = pred2$sigma[k], nu = pred2$nu[k], 
                    tau = pred2$tau[k])
