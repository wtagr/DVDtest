...
#' @param permat.all a result of \code{make.perm}
...
wass_perm <-
function(vdFun, nperm, ydata1, ydata2, eval.index.grid, permat.all, ..., exclude,
  ...
  if (mc.cores == 1) {
    permtmp <- lapply(1:length(ydata1), wass.perm, vdFun = vdFun, dat1 = ydata1,
                      dat2 = ydata2, ..., permat = permat.all, .index = eval.index.grid, 
